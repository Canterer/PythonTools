...
# Bind Lind等特殊关键字怎么排除?? 包含特殊关键字的结点(其不可省略Field)

# 子类的初始化参数不同 如何统一????
# 标识每一个节点的必要参数信息  必然在规则中不会缺失(准确指明一个结点必然具备必要参数)

# 绑定结点的返回字段 用于初始化其它节点的参数
# 在函数还未运行时,就可以绑定返回字段
...
class BaseNode:
	...
	def __init__(self, nodeName, instanceName):
		...
		self.instanceName = instanceName# 实例化对象名
		self.initNodeInstance()

		# 参数和返回值 绑定NodeSlot对象
		# NodeSlot对象是为了绑定结点间数据关系(未能取值时先绑定关系)
		...
		# 函数内采用封闭作用域 没有全局变量结点的概念

		self.moveStep = 0# 对应的前进步数 对应规则读取数据的实际消耗 为实现默认参数机制
	...
	def getHandlesList(cls):
		if not hasattr(cls, "handlesList"):
			cls.handlesList = []
	...
	def linkNodeField(self, fieldName, nextNode):
	...
	def getInsMoveStepByRule(cls, keyWord):
		moveStep = 0
		if type(keyWord) is int:# 默认实例名为数字 其值无意义仅为区分 所以采用数字
			moveStep = 1
		return moveStep
...
class OperateNode(ValueNode):
	...
	def calculate(self):
		...
		elif operateType == "Equal":
			l = self.getArgValue("Left")
			r = self.getArgValue("Right")
			self.setValue(l == r)
		elif operateType == "Less":
			l = self.getArgValue("Left")
			r = self.getArgValue("Right")
			self.setValue(l < r)
...
#局部变量仅在函数内部有意义
#函数进入和退出 设置局部变量栈序号
class CompositeNode(CalculateNode):
	def __init__(self, nodeName, instanceName, argsList=[], retsList=[], handlesList=[]):
		BaseNode.SubNodeClassDic.setdefault(nodeName, self)#记录函数名 对应的实例对象
		print("CompositeNode nodeName:{0} instanceName:{1}".format(nodeName, instanceName))
		super(CompositeNode, self).__init__(nodeName, instanceName, argsList, retsList, handlesList)
	...
	def calculate(self):
		for fieldName in self.getArgsList():#函数参数初始化
...
class LocalValue(ValueNode):
	...
	def setValue(self, value):
		...
		# 	print("LocalValue instanceName:{0} is exist!!! valueName key is argName")
