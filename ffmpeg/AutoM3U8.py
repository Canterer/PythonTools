...
def getCmd(inputFileName, outputSubDir, segment_type_flag, RateScale):
    if "." not in inputFileName:
        if outputSubDir is None:
            outputSubDir = inputFileName
        inputFileName = inputFileName + ".mp4"
    if RateScale is None:
        RateScale = 1
    return runCmd(inputFileName, outputSubDir, segment_type_flag, RateScale*DefaultVideoBite, RateScale*DefaultMaxRate, RateScale*DefaultMinRate)

def runCmd(inputFileName, outputSubDir, segment_type_flag, videoBite=DefaultVideoBite, maxRate=DefaultMaxRate, minRate=DefaultMinRate):
    print("runCmd args:\n inputFileName:{0} outputSubDir:{1} segment_type_flag:{2} videoBite:{3} maxRate:{4} minRate:{5}".format(inputFileName, outputSubDir, segment_type_flag, videoBite, maxRate, minRate))
    VideoBiteStr = str.format(" -b:v {0}M -maxrate:v {1}M -minrate:v {2}M -bufsize:v {3}M ", videoBite, maxRate, minRate, 2*maxRate)
    cmdStr = ffmpegExePath + " -i " + os.path.join(inputRootDir,inputFileName)
...
def main(argv):
    ...
    opts, args = getopt.getopt(argv,"i:f:r:",["outputSubDir=","segement_type_flag=","rateScale="])
